...
- **Hamlib (`rigctld`)** OR **`flrig`** installed and configured for your specific radio.
  - _Note: `rigctld` is part of the `hamlib` package._
  - _Note: `flrig` is a standalone GUI program._
...
_NOTE: The logic enforces `< 10 MHz = LSB` and `>= 10 MHz = USB`, except for 60m (USB)._
